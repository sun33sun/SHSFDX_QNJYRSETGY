...
public class QuestionManager : SerializedMonoBehaviour
{
    ...
    public GameObject _提交按钮;
    public List<float> _得分 = new List<float>();
    public string _读取Json路径;
    ...
    [Space(10)]
    public List<QuestionController> _考题组;
    private GameObject QuestionPrefab;//没有把这个鬼东西改为自动查找预制体，目的是为了如果有多种款式的界面 可以自己选择  结果这个想法也没等到扩展

    List<QuestionTool> QuestionToolList = new List<QuestionTool>();
    ...
    public void Awake()
    {
        ...
        _提交按钮.transform.SetAsLastSibling();
    }
    ...
    void ReadJson()
	{
        string json = Resources.Load<TextAsset>("QuestionPrefabs/" + _读取Json路径).text;
        _考题组 = JsonConvert.DeserializeObject<List<QuestionController>>(json);
    }
}
